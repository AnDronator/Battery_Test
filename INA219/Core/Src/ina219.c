...
void setCalibration_32V_2A(void) {
	...
	// MaximumLSB = MaxExpected_I/512
    // MaximumLSB = 0,0059             (5900uA per bit)

    // 4. Choose an LSB between the min and max values
    //    (Preferrably a roundish number close to MinLSB)
    // CurrentLSB = 0,0059 (5,9 mA per bit)
    ...
    // PowerLSB = 0.4071 (400mW per bit)

    // 7. Compute the maximum current and shunt voltage values before overflow
    //
     // ну хер значет наверно правильно так: Max_Current = Current_LSB * 32767
    // Max_Current = Current_LSB * 512
    // Max_Current = 3A before overflow
    //
    ...
    ina219_currentDivider_mA = 12; // Current LSB = 5900uA per bit (1000/5900 = 10)
...
float getCurrent_mA() {
	uint16_t valueDec = getCurrent_raw();
